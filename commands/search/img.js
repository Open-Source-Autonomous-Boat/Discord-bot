...
let pixabay_api_key = process.env.PIXABAY_API_KEY;
...
module.exports = class PixabayCommand extends commando.Command {
    ...
    async run(msg, args) {
        ...
        if (args.length < 1) {
            ...
            let url_encoded_search_query = search_term.split(" ").join("%20")
            console.log(search_term)
            let pixabay_link = `https://pixabay.com/api/?key=${pixabay_api_key}&q=${url_encoded_search_query}&image_type=photo`
            ...
            fetch(pixabay_link)
                ...
                .then((out) => {
                    ...
                    if (out.totalHits === 0) {
                        ...
                        // let half_results_length = Math.floor(out.hits.length / 2)
                        let randomNumber;
                        if (out.totalHits < 10) {
                        ...
                        let random_img_link = out.hits[randomNumber].largeImageURL;
